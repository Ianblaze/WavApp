...
class _PhoneAuthScreenState extends State<PhoneAuthScreen> {
  ...
  Future<void> _sendOtp() async {
    ...
    if (number.isEmpty) { AuthSnackBar.show(context, 'Enter your phone number'); return; }
    if (!_rateLimiter.allow()) {
      AuthSnackBar.show(context, 'Wait $_cooldownSeconds seconds before resending');
      return;
    }
    ...
    await context.read<AuthProvider>().signInWithPhone(
      ...
      onError: (err) {
        if (mounted) { setState(() => _loading = false); AuthSnackBar.show(context, err); }
      },
    );
  }
  ...
  Future<void> _verifyOtp() async {
    ...
    if (otp.length != 6) { AuthSnackBar.show(context, 'Enter the 6-digit code'); return; }
    if (_verificationId == null) { AuthSnackBar.show(context, 'Request an OTP first'); return; }
    setState(() => _loading = true);
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        labelStyle: TextStyle(
          ...
          fontWeight: FontWeight.w400, color: Colors.white70, letterSpacing: 0,
        ),
        floatingLabelStyle: TextStyle(
          fontFamily: 'Circular', fontSize: scaledFont * 0.55,
          fontWeight: FontWeight.w700, color: _cardHotPink, letterSpacing: 0,
        ),
